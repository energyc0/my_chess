			...
			for (int x = 0; x < 8; x++)
			{
				...
				else
				{
					if (x % 2 == 0)
					{
						if (y % 2 == 0)
						{
							chessBoard::drawWhite(WhiteCell, WhiteCell);	// пуста€ бела€ клетка
						}
						else
						{
							chessBoard::drawBlack(BlackCell, BlackCell);	// пуста€ чЄрна€ клетка
						}
					}
					else
					{
	...
	while (true)
	{
		...
		else
		{
			...
			if (field[FcordX][FcordY]->returnTeam() == teamEmb)	// проверка на соответствие команды игрока и фигуры
				...
				if (field[FcordX][FcordY]->returnEmblem() == BlackCell || field[FcordX][FcordY]->returnEmblem() == WhiteCell)	// если пуста€ клетка
